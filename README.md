...
- 批量/定时创建：可勾选多个 Apple 登录态；一次创建会让选中账号同时跑一轮，定时默认 60 分钟，失败账号只在本次定时创建中临时跳过，下一次再试。
...
4. 点击 `提交新接口验证码`。
5. 后端会保存独立的新接口登录态，包含 `scnt`、管理接口 Cookie、动态 `apiKey` 等必要字段。
...
定时创建默认间隔为 `60` 分钟。`定时每次轮数` 表示每次定时任务最多跑多少轮；每一轮都会让当前勾选账号同时创建。没有总数输入，面板旁边只记录累计成功/失败数量。若某个账号在本次定时创建里达到上限或创建失败，只会临时跳过这个账号，其他账号继续跑；下一次定时创建会重新尝试它。
